...
function queryVideo(cids, callback) {
	...
	var url =
		...
		querystring.stringify({
			...
			id: cids,
			fields:
				"items(id,contentDetails/duration,snippet(title,thumbnails/default),status/uploadStatus)",
			key: key,
		});
	...
	var req = https.get(url, function (res) {
		...
		res.on("end", function () {
			try {
				...
				callback("StringNotJSON");
				return;
			}
			if (errorAPIHandler(str, callback)) {
				return;
			}
			var out = {};
			...
			for (var item in str.items) {
				...
				if (item.contentDetails.duration.indexOf("W") > -1) {
					...
					if (days) {
						item.contentDetails.duration = item.contentDetails.duration.replace(
							days[0],
							(parseInt(days[1]) + weeks * 7).toString() + "D"
						);
					} else {
						item.contentDetails.duration = item.contentDetails.duration.replace(
							"T",
...
function errorAPIHandler(str, callback, autocallback = true) {
	if (str.error) {
		var errorstr;
		var apilimit;
		for (var item in str.error.errors) {
			if (item.reason == "quotaExceeded") {
				errorstr += "API Limit\n";
				apilimit = true;
			} else {
				errorstr += item.message;
			}
			console.log(errorstr);
			if (autocallback && apilimit) callback("RateLimitReached");
			return apilimit;
		}
	}
	return false;
...
YT.prototype.getVideo = function (inCid, callback) {
	if (typeof callback !== "function") {
		log.error("YT.getVideo expects second argument to be a callback");
		return;
	}
	var out = {};
	...
	if (Array.isArray(inCid)) {
		...
		if (typeof inCid !== "string") {
			return callback("InvalidCid");
		}
		if (inCid.indexOf(",") > -1) {
...
YT.prototype.search = function (query, callback) {
	var inObj = {
		...
		fields: "items(id)",
		key: nconf.get("apis:YT:searchkey") ? nconf.get("apis:YT:searchkey") : key,
	};

	if (nconf.get("apis:YT:restrictSearchToMusic")) inObj.videoCategoryId = 10; // This is restricting the search to things categorized as music

	var url =
		"https://www.googleapis.com/youtube/v3/search?" +
		querystring.stringify(inObj);
...
YT.prototype.getPlaylist = function (data, callback) {
	var url =
		...
		querystring.stringify({
			...
			fields:
				"nextPageToken,prevPageToken,items(snippet(title,thumbnails(medium(url)),resourceId(videoId)))",
			key: key,
		});
	...
	https
		.get(url, function (res) {
			...
			res.on("end", function () {
				...
				if (str.error) {
					if (errorAPIHandler(str, callback, false)) {
						callback("RateLimitReached");
						return;
					} else {
...
YT.prototype.getChannelPlaylists = function (data, callback) {
	var url =
		...
		querystring.stringify({
			...
			pageToken: data.pageToken,
			fields:
				"pageInfo,nextPageToken,items(id,snippet(title,thumbnails(medium(url))),contentDetails(itemCount))",
			key: key,
		});
...
YT.prototype.findChannels = function (data, callback) {
	var url =
		...
		querystring.stringify({
			...
			type: "channel",
			fields:
				"nextPageToken,prevPageToken,items(snippet(channelId,title,thumbnails(medium(url)))),pageInfo",
			key: key,
		});
...
YT.prototype.findPlaylists = function (data, callback) {
	var url =
		...
		querystring.stringify({
			...
			type: "playlist",
			fields:
				"nextPageToken,prevPageToken,pageInfo,items(id(playlistId),snippet(title,thumbnails(medium(url))))",
			key: key,
		});
	...
	https
		.get(url, function (res) {
			...
			res.on("end", function () {
				...
				if (str.items.length == 0) callback("NoPlaylistsFound");
				else callback(null, str.items[0].snippet.title);
			});
		})
		.on("error", function (err) {
			callback("ConnectionError");
